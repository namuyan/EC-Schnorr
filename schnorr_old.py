...
def supplicant(a):
   ...
   """
    ...
    print("\tXx: ", hex(x.x))
    print("\tXy: ", hex(x.y))
    aChannel.send(str(x.x).encode('utf8'))
    import time
    time.sleep(1)
    aChannel.send(str(x.y).encode('utf8'))
    print("\n<- X sent.")
    ...
    # send  y
    # print "\n<- sending y."
    aChannel.send(str(y).encode('utf8'))
    print("\n<- y sent.")
...
def authenticator(v):
   ...
   """
    ...
    print("\n*  e generated.\n\t e: " + hex(e))
    # send e
    bChannel.send(str(e).encode('utf8'))
    print("\n<- e sent.")

    # receive y
    y = bChannel.recv(dataSize).decode('utf8')
    y = int(y)
...
def main():
    ...
    elif (("--generate-keys" in sys.argv) or
          ("-gk") in sys.argv) and (("--supplicant" in sys.argv) or ("-s" in sys.argv)) or (
              (("--generate-keys" in sys.argv) or ("-gk") in sys.argv) and
              (("--authenticator" in sys.argv) or
               ("-a" in sys.argv))) or ((("--supplicant" in sys.argv) or ("-s" in sys.argv)) and
